...
// --- NEW: Octree for environment objects ---
...
self.onmessage = (e) => {
    ...
    switch (type) {
        case 'init':
            config = payload;
            spawnedCount = 0;
            // Initialize the Octree with world bounds
        ...
        case 'updateGameState':
            gameState = payload;
            // Rebuild the Octree with the latest obstacle data
            environmentOctree.clear();
            if (payload.obstacles) {
                payload.obstacles.forEach(obs => {
                    const size = 5; // A generic size for obstacles
